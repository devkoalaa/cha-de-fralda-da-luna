      ...
      <div className="flex flex-col items-center justify-between font-[family-name:var(--font-geist-sans)] shadow-2xl rounded-xl w-full m-0.8">
        <div className="top-0 left-0 w-full flex flex-col items-center px-9">
          ...
          <div className="hidden lg:flex w-full max-w-screen-xl mx-auto justify-between items-center bg-marronzim h-10 rounded-md divide-x shadow-lg select-none">
            {/* Os itens do menu */}
            <div
              className={`flex-auto text-center text-white px-2 cursor-pointer hover:brightness-75 relative ${screenSelected === 'main' ? 'after:content-[""] after:absolute after:bottom-[-8px] after:left-0 after:right-0 after:h-[2px] after:bg-white' : ''}`}
              onClick={() => handleScreen('main')}
            >
              Home
            </div>
            <div
          ...
          <AnimatePresence>
            {menuOpen && (
              ...
              >
                <div
                  className={`w-full text-center text-white px-2 py-2 cursor-pointer ${screenSelected === 'main' ? 'bg-verde' : ''}`}
                  onClick={() => handleScreen('main')}
                >
                  Home
                </div>
